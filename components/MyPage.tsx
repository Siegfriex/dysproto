        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start text-center md:text-left">
            ...
            <div className="flex-1">
                <div className="flex items-center justify-center md:justify-start gap-2 md:gap-3 mb-2">
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Kim Design</h1>
                    <span className="px-2 py-0.5 bg-primary-50 text-primary-600 text-[10px] md:text-xs rounded-lg font-bold border border-primary-100">Pro</span>
                </div>
                <p className="text-slate-500 mb-4 md:mb-6 max-w-md leading-relaxed text-sm md:text-base">Junior UI/UX Designer focused on minimalist interfaces and accessible color systems.</p>
            ...
            <div className="flex gap-6 md:gap-10 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-10 w-full md:w-auto justify-center md:justify-start">
                <div className="text-center">
                    <div className="text-2xl md:text-3xl font-bold text-slate-900">24</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-3 md:space-y-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="flex gap-4 md:gap-5 items-center group cursor-pointer p-2 md:p-3 hover:bg-slate-50 rounded-2xl transition border border-transparent hover:border-slate-100">
                            <img src={`https://picsum.photos/100/100?random=${i+10}`} className="w-12 h-12 md:w-16 md:h-16 rounded-xl object-cover shadow-sm" alt="Work" />
                            <div className="flex-1">
                                <h4 className="text-slate-900 font-bold mb-0.5 text-sm md:text-base">Portfolio Update v{i}</h4>
                                <p className="text-[10px] md:text-xs text-slate-400 font-medium">Analyzed 2 days ago</p>
                            </div>
                            <div className="text-right">
                                <div className="text-lg md:text-xl font-bold text-primary-500">8{i}</div>
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                {['Modern Web', 'App Inspirations', 'Typography Ideas', 'Archive'].map((folder, i) => (
                    <div key={i} className="aspect-square bg-slate-50 rounded-2xl md:rounded-3xl border border-slate-100 hover:border-primary-300 hover:shadow-md transition-all cursor-pointer flex flex-col items-center justify-center group">
                        ...
                        <span className="text-xs md:text-sm text-slate-700 font-bold group-hover:text-primary-700 transition-colors text-center px-2">{folder}</span>
                        <span className="text-[10px] md:text-xs text-slate-400 mt-1 font-medium">{3 + i} items</span>
                    </div>
                ))}
            </div>
        </div>
